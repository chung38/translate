...
export const useTranslation = (
  ...
  const translateBatch = async (texts: string[], targetLangs: string[], industry: string, retryCount = 0): Promise<Record<string, string>[]> => {
    ...
    try {
      ...
      5. **極度重要：輸入文字中可能包含 [f0]...[/f0]、[f1]...[/f1] 等標籤，這些是排版標記。請務必在翻譯時原封不動地保留這些標籤，並將翻譯後的文字放在標籤內對應的位置。如果原文有標籤，譯文就必須有完全相同的標籤！請勿修改、刪除或轉義這些標籤（例如不要把中括號變成其他符號）。**
      6. **極度重要：請務必保留標籤與標籤之間、以及標籤與文字之間的「空白字元 (空格)」，確保翻譯後的單字不會錯誤地連在一起！**
      7. 不要包含任何 Markdown 標籤（如 \`\`\`json）或額外文字，只回傳純 JSON 字串。
